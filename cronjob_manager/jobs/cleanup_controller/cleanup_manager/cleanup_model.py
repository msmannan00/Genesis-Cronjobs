...
class cleanup_model(request_handler):
    ...
    def __start_cleanup(self):
        ...
        elastic_controller.get_instance().invoke_trigger(ELASTIC_CRUD_COMMANDS.S_DELETE, [ELASTIC_REQUEST_COMMANDS.S_DEL_CLEAN, [helper_method.get_time()-30], [None]])
        log.g().i(CLEANUP_MESSAGES.S_CLEANUP_MID_MONTHLY)
        elastic_controller.get_instance().invoke_trigger(ELASTIC_CRUD_COMMANDS.S_UPDATE, [ELASTIC_REQUEST_COMMANDS.S_CLEAN_MONTHLY_SCORE, [helper_method.get_time()-14], [False]])
